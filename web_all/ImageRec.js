...
let animals = [
    ...
    "Dog 白冰冰 2024/08/05 reward:8000",
...
function compareRecognitionResult(recognizedName) {
    ...
    if (matchedPet) {
        ...
        document.getElementById('recognitionResult').innerHTML = `
            ...
            <h5>辨識結果：</h5>
            <p>名字：${matchedPet.name}</p>
            <p>類別：${matchedPet.type}</p>
            <p>走失日期：${matchedPet.date}</p>
            <p>賞金：${matchedPet.reward}</p>
        `;
    } else {
